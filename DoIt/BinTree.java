...
public class BinTree<K,V> {
	...
	private Comparator<? super K> comparator = null; //키 값의 대소관계를 비교하는 비교자
	
	
	//생성자
	public BinTree() {//비어있는 이진검색 트리 생성
		root=null;
	}
	...
	public V search(K key) {
		...
		while(true) {
			if(p==null)				//루트가 널이면
				return null;		//검색 실패
			int cond=comp(key, p.getKey());	//key와 노드p의 키 비교
			if(cond==0)
				return p.getValue();	//검색 성공
			else if(cond<0)
				p=p.left;			//key쪽이 작으면 왼쪽 서브트리에서 검색
			else if(cond>0)
				p=p.right;			//key쪽이 크면 오른쪽 서브트리에서 검색
		}
	}
	...
	private void addNode(Node<K,V> node, K key, V data) {
		...
		if(cond==0)
			return;			//key가 이진검색트리에 이미 있는 경우
		else if(cond<0) {
			if(node.left==null)
				node.left = new Node<K,V>(key, data, null, null);//(key,data,left,right)
			else
				addNode(node.left,key,data);	//왼쪽 서브트리에 추가
		}else {
			if(node.right==null)
	...
	public boolean remove(K key) {
		...
		boolean isLeftChild=true;//p는 부모의 왼쪽자식노드인지
